...
class Seq2Seq(nn.Module):
    ...
    def beam_decode(self, inputs, beam_size=100, max_length=3):
        
        #encode inputs
        score, out, (h, c), output_seq = self(inputs, max_length=3, encode_only=True)
        ...
        for l in range(max_length + 1):
            ...
            for output_seq, score, state in beam:
                new_beam.extend(self.all_possible_from_one_step(l, output_seq, score, state))
    ...
    def all_possible_from_one_step(self, l, output_seq, score, state):
        ...
        return self.decode_one_step(l, output_seq, score, state)
        
    def decode_one_step(self, t, output_seq, score, state):
        h, c = state[-1]
